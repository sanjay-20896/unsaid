    ...
    return(
        <>
         <div className="signIn forgotPassword">
            <h1 style={{animationDelay:"0.1s"}} className="greet canelaThin font20 fadeLeftToRightAnimation">{t('forgotYourPassword')}</h1>
            <div style={{animationDelay:"0.2s"}} className="signInContent anoHalfRegular font16 fadeLeftToRightAnimation">{t('forgotPasswordDesc')}</div>
            <div className="inputComponentForm">
                <div style={{animationDelay:"0.3s"}} className="signInEmail fadeLeftToRightAnimation"><Input fpMsg={fpMsg} error={fpEmailTouched && fpEmailError} errorMsg={"Invalid email address!"} type="email" placeHolder={t('email')} onChangeValue={onFpEmailChange} value={fpEmail} required={true}/></div>
                <button style={{animationDelay:"0.4s"}} className={`btn width-100 signInSubmitButton anoRegular font20 white fadeLeftToRightAnimation ${fpEmailError?"btnInactive":"btnPrimary"}`} onClick={()=>handleSubmitFP()}>{btnContentFP}</button>
            </div>
        </div>
        <style jsx>{`
                .dobError{
