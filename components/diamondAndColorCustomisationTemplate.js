...
function customizeViews(props) {
    ...
    function getCompleteImageSrc(variantId){
        ...
        let customisationData = productsCustomisationData.find(item=>item.variantId==variantId)
        if(!!customisationData)
            return getImageUrl(customisationData.completeProductImage,600)
        else
            return false
    }
    function selectCustomColorFun(id){
        //console.log("selectCustomColorFunFun",id);
        alert("dlsjds")
    ...
    function setVariantBasedOnDiamondsAndColor(numberOfDiamonds,colorText){
        ...
        if(product.color.color_text==colorText && product.number_of_diamonds_text==numberOfDiamonds){
            ...
            if(Array.isArray(product.relatedProducts)){
                ...
                let relatedProducts = product.relatedProducts.filter(p=>p.relation=="variant")
    ...
    function showColorSelector(){
        ...
        allProducts = allProducts.concat(relatedProducts).filter(p=>p.relation=="variant")
        let currentSelectedProduct = variantProduct || props.product
        let allColorProducts = allProducts.filter(p=>p.number_of_diamonds_text==currentSelectedProduct.number_of_diamonds_text)
        if(Array.isArray(allColorProducts) && allColorProducts.length == 3){
            //rearrange all color products based on currentSelectedProduct
            let centerProductIndex = allColorProducts.findIndex(p=>p.color.color_text==currentSelectedProduct.color.color_text)
            if(centerProductIndex!=-1 && centerProductIndex!=1){
                let centerProduct = allColorProducts[centerProductIndex]
                let middleProduct = allColorProducts[1]
                allColorProducts[1] = centerProduct
                allColorProducts[centerProductIndex] = middleProduct
            }
        }
        setTransform2(0)
        setCurrentIndex(1)
    ...
    let currentSelectedProductCustomisationData = Array.isArray(productsCustomisationData) && !!currentSelectedProduct?productsCustomisationData.find(item=>item.variantId==currentSelectedProduct.product):null
    ...
    // console.log('customisationData',productsCustomisationData)
    // console.log('currentSelectedProductCustomisationData',currentSelectedProductCustomisationData)
    if(Array.isArray(allProducts)){
        for(let i=0;i<allProducts.length;i++){
            let customisationData = productsCustomisationData.find(item=>item.variantId==allProducts[i].product)
            if(!numberAndDiamondImageMapping[allProducts[i].number_of_diamonds_text])
    ...
    // console.log("numberOfDiamonds",numberOfDiamonds)
    ...
    // console.log("filteredProducts",colorProducts)
    ...
    // console.log('productsOfSameColorAsCurrent',productsOfSameColorAsCurrent)
    // get other colors for same diamond count
    let allColorProducts = allProducts.filter(p=>p.number_of_diamonds_text==currentSelectedProduct.number_of_diamonds_text)
            ...
            <div className="firstWindow">
                <div style={{height:`${layerHeight}px`}} className="allLayers positionRelative alignCenter">
                    ...
                    {productsOfSameColorAsCurrent.map(p=>{
                        let imgSrc = getCompleteImageSrc(p.product)
                        if(imgSrc){
                            return (
                    ...
                    <div style={{height:'100%'}} className="positionRelative makeCenterInMobile">
                        <div className="hideForMobile">
                            <div className="colorTransform">
                                {Array.isArray(allColors) && allColors.map((p,index)=>{
                                    let v1 = getProductImage(p,"v1","standard")
                                    return(
                                        <div className={`colorImage ${currentIndex===index?"current":""}`} onClick={()=>selectCustomColorFunFun(index)}>
                        ...
                        <div className="showForMobile">
                            <div className="colorTransform2">
                                <SwipeableViews style={{padding:"0 10rem 0 10rem"}} slideStyle={{padding:"0 0 0 0"}} index={indexSlider} onChangeIndex={(index)=>selectCustomColorFunFun(index)} springConfig={springConfig}>
                                    {Array.isArray(allColors) && allColors.map((p,index1)=>{
                                    let v1 = getProductImage(p,"v1","standard")
                                    // {props.customColors.map((colorImg,index1)=>{
                                        return(
                                            <div className={`colorImage ${indexSlider===index1?"current":""}`} onClick={()=>selectCustomColorFunFun(index1)}>
                                                <img className="width-100" src={v1}/>
                                            </div>
                                        )
                                    })}
                    ...
                    <div className="hideForMobile">
                        <div className="positionRelative newDC">
                            <div className="diamondCountAndColor">
                                ...
                                <div onClick={()=>showColorSelector()} className="colorName">
