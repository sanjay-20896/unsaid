...
function generatePaths(paths){
    ...
    if(Array.isArray(paths)){
        for(let i=0;i<paths.length;i++){
            let createdPath;
                for(let j=0;j<paths[i].filterValues.length;j++){
                    createdPath=[`${paths[i].filterType}`,`${paths[i].filterValues[j].uri}`]  
                    generatedPath.push(createdPath)
                }
        }
...
export async function getStaticPaths({locales}){
    ...
    for(let i=0;i<locales.length;i++){
        if(locales[i]=="fr"){
            let commonProps=await getDynamicMenuAndFooterInfo("fr");
            let plpData=commonProps.commonData.plpPaths;
            let configPaths=generatePaths(plpData);
            if(Array.isArray(configPaths)){
                for(let i=0;i<configPaths.length;i++){
                    paths.push({params:{slug:configPaths[i]},locale:"fr"})
                }
            }
        }else{
            let commonProps=await getDynamicMenuAndFooterInfo("en");
            let plpData=commonProps.commonData.plpPaths;
            let configPaths=generatePaths(plpData);
            if(Array.isArray(configPaths)){
                for(let i=0;i<configPaths.length;i++){
                    paths.push({params:{slug:configPaths[i]},locale:"en"})
                }
            }
        }
